...
bool SparkDataControl::toggleLooperAppMode() {
    ...
    bool looperActive = (newSubMode == SUB_MODE_LOOPER || newSubMode == SUB_MODE_SPK_LOOPER ||
                         newSubMode == SUB_MODE_LOOP_CONTROL || newSubMode == SUB_MODE_LOOP_CONFIG);
    Serial.printf("LOOPER_MODE active=%d submode=%d\n", looperActive ? 1 : 0, newSubMode);
    return true;
}
...
bool SparkDataControl::blockCurrentAmp() {
    string blockedAddress;
    if (!bleControl || !bleControl->blockConnectedAmp(blockedAddress)) {
        if (sparkDisplay) {
            sparkDisplay->showTemporaryMessage("NO AMP CONNECTED");
        }
        delay(1500);
        return false;
    }

    string addressSuffix = blockedAddress;
    if (addressSuffix.length() > 8) {
        addressSuffix = addressSuffix.substr(addressSuffix.length() - 8);
    }
    if (sparkDisplay) {
        sparkDisplay->showTemporaryMessage("AMP BLOCKED", addressSuffix);
    }
    delay(2000);
    restartESP(false);
    return true;
...
bool SparkDataControl::clearBlockedAmp() {
    return bleControl && bleControl->clearBlockedAmp();
}
...
void SparkDataControl::setAmpParameters() {
    ...
    Serial.printf("HWINFO banks=%d amp=\"%s\"\n",
                  SparkPresetControl::getInstance().numberOfHWBanks(),
                  ampName.c_str());
}
...
bool SparkDataControl::storeHWPreset(int preset) {

    currentMsg = sparkMsg.storeHardwarePreset(nextMessageNum, preset);
    return triggerCommand(currentMsg);
}
...
void SparkDataControl::handleIncomingAck() {
    ...
    if (lastAck.cmd == 0x04) {
        ...
        if (lastAck.subcmd == 0x27) {
            DEBUG_PRINTLN("Received ACK for 0x27 command");
            Serial.println("OK STOREHW");
        }
