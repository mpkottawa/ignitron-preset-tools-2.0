...
unsigned long SparkButtonHandler::bankComboStarted_ = 0;
bool SparkButtonHandler::bankComboActive_ = false;
bool SparkButtonHandler::bankComboHandled_ = false;
...
void SparkButtonHandler::readButtons() {
    const bool bothBankButtonsPressed = isBankComboPressed();

    if (bothBankButtonsPressed) {
        if (!bankComboActive_) {
            bankComboActive_ = true;
            bankComboHandled_ = false;
            bankComboStarted_ = millis();
            resetBankButtons();
        }

        if (!bankComboHandled_ && millis() - bankComboStarted_ >= 2000) {
            bankComboHandled_ = true;
            if (spark_dc_ && spark_dc_->operationMode() == SPARK_MODE_APP) {
                spark_dc_->blockCurrentAmp();
            }
        }
        return;
    }
...
bool SparkButtonHandler::isBankComboPressed() {
    return digitalRead(BUTTON_BANK_UP_GPIO) == HIGH &&
           digitalRead(BUTTON_BANK_DOWN_GPIO) == HIGH;
}

void SparkButtonHandler::resetBankButtons() {
    btn_bank_down_ = BfButton(BfButton::STANDALONE_DIGITAL, BUTTON_BANK_DOWN_GPIO, false, HIGH);
    btn_bank_up_ = BfButton(BfButton::STANDALONE_DIGITAL, BUTTON_BANK_UP_GPIO, false, HIGH);
    if (spark_dc_) {
        configureButtons();
    }
}
...
OperationMode SparkButtonHandler::checkBootOperationMode() {
    ...
    if (digitalRead(BUTTON_PRESET3_GPIO) == HIGH) {
