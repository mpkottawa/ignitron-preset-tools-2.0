...
#if defined(OLED_DRIVER_SSD1306) || defined(OLED_DRIVER_SSD1309)
...
void SparkDisplayControl::setSerialDisplayMirror(bool enabled) {
    serialDisplayMirrorEnabled_ = enabled;
    lastSerialDisplayMirrorTimestamp_ = 0;
}

void SparkDisplayControl::setSerialTunerStream(bool enabled) {
    serialTunerStreamEnabled_ = enabled;
    lastSerialTunerStreamTimestamp_ = 0;
}

void SparkDisplayControl::init(int mode) {
    // Larger SSD1309 OLED modules can need a little longer than the ESP32
    // to finish power-up; otherwise they may show a striped/garbled buffer.
    delay(250);

#if defined(OLED_DRIVER_SSD1306) || defined(OLED_DRIVER_SSD1309)
    // SSD1306_SWITCHCAPVCC = generate display voltage from 3.3V internally.
    // SSD1309 128x64 I2C modules use the same command path in Adafruit_SSD1306.
    if (!display_.begin(SSD1306_SWITCHCAPVCC, 0x3C) &&
        !display_.begin(SSD1306_SWITCHCAPVCC, 0x3D)) {
        Serial.println(F(
#if defined(OLED_DRIVER_SSD1309)
            "SSD1309 initialization failed"
#else
...
void SparkDisplayControl::showTemporaryMessage(const string &line1, const string &line2) {
    display_.clearDisplay();
    display_.setTextColor(WHITE);
    display_.setTextWrap(false);
    display_.setTextSize(1);
    drawCentreString(line1.c_str(), 18);
    if (!line2.empty()) {
        drawCentreString(line2.c_str(), 38);
    }
    display_.display();
}
...
void SparkDisplayControl::update(bool isInitBoot) {
    ...
    // logDisplay();
    display_.display();
    emitSerialDisplayMirror(opMode, subMode);
    emitSerialTunerStream();
}
...
string SparkDisplayControl::serialSafeText(string value) {
    for (char &ch : value) {
        if (ch == '|' || ch == '\r' || ch == '\n' || ch == '"') {
            ch = ' ';
        }
...
void SparkDisplayControl::emitSerialDisplayMirror(OperationMode opMode, SubMode subMode) {
    if (!serialDisplayMirrorEnabled_) {
        return;
    }
    unsigned long now = millis();
    if (now - lastSerialDisplayMirrorTimestamp_ < serialDisplayMirrorInterval_) {
        return;
    }
    lastSerialDisplayMirrorTimestamp_ = now;

    uint8_t *buffer = display_.getBuffer();
    if (buffer) {
        const char hex[] = "0123456789ABCDEF";
        Serial.print("OLED_HEX ");
    ...
    string line0 = currentBTModeText;
    string line1 = "";
    string line2 = "";
    string line3 = "";

    if ((opMode == SPARK_MODE_APP) && sparkDC_->isInitBoot()) {
        line0 = "IGNITRON";
        line1 = "APP v" + VERSION;
    } else if (opMode == SPARK_MODE_KEYBOARD) {
        line0 = upperButtonsShort;
        line1 = upperButtonsLong;
        line2 = lowerButtonsLong;
        line3 = lowerButtonsShort;
    } else if (subMode == SUB_MODE_TUNER) {
        line0 = "TUNER";
        line1 = currentNote;
        line2 = "cents " + to_string(noteOffsetCents);
    } else {
        SparkPresetControl &presetControl = SparkPresetControl::getInstance();
        int pendingBank = presetControl.pendingBank();
        int activePreset = presetControl.activePresetNum();
        line0 = currentBTModeText + " B" + to_string(pendingBank) + " P" + to_string(activePreset);
        line1 = primaryLineText;
        line2 = secondaryLineText;
        if (subMode == SUB_MODE_LOOP_CONTROL || subMode == SUB_MODE_LOOP_CONFIG) {
            line3 = "Looper";
        }
    }
    ...
    Serial.print("OLED|");
    Serial.print(serialSafeText(line0).c_str());
    Serial.print("|");
    Serial.print(serialSafeText(line1).c_str());
    Serial.print("|");
    Serial.print(serialSafeText(line2).c_str());
    Serial.print("|");
    Serial.println(serialSafeText(line3).c_str());
}

void SparkDisplayControl::emitSerialTunerStream() {
    bool physicalTunerActive = sparkDC_ && sparkDC_->subMode() == SUB_MODE_TUNER;
    if (!serialTunerStreamEnabled_ && !physicalTunerActive) {
        return;
    }
    unsigned long now = millis();
    if (now - lastSerialTunerStreamTimestamp_ < serialTunerStreamInterval_) {
        return;
    }
    lastSerialTunerStreamTimestamp_ = now;

    Serial.print("TUNER note=");
    Serial.print(serialSafeText(currentNote).c_str());
    Serial.print(" cents=");
    Serial.println(noteOffsetCents);
}
