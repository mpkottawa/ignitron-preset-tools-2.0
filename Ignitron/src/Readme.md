...
|---|---:|---|---|
| Adafruit SSD1306 | 2.5.13 | SSD1306/SSD1309 display control | Arduino IDE Library |
...
The SSD1306 and SH110X drivers are already linked in the Libraries section of the platform.ini file, but the active OLED controller needs to be selected in code since the drivers use different handles and somewhat different API. The 2.42" 128x64 SSD1309 display uses the SSD1306-compatible command path.

Go to [src/Config_Definitions.h](Config_Definitions.h) and enable **one of** `#define OLED_DRIVER_SSD1306`, `#define OLED_DRIVER_SSD1309`, `#define OLED_DRIVER_SH1106`, or `#define OLED_DRIVER_SH1107` by moving the `//` at the beginning of the line to the definitions that should be **disabled**.
