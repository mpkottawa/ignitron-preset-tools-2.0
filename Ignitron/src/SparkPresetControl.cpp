...
void SparkPresetControl::updateActiveWithPendingPreset() {
    ...
    Serial.printf("REMOTE_PRESET bank=%d preset=%d hwbank=%d\n",
                  activeBank_, activePresetNum_, activeHWBank_ + 1);
}
...
void SparkPresetControl::setHWBank(int i) {
    if (i < 0 || i >= presetBuilder.numberOfHWBanks())
        return;
    activeHWBank_ = pendingHWBank_ = i;
}
...
void SparkPresetControl::updatePendingBankStatus() {
    ...
    //}
    Serial.printf("REMOTE_BANK bank=%d hwbank=%d\n", pendingBank_, pendingHWBank_ + 1);
    if (operationMode == SPARK_MODE_AMP) {
        activeBank_ = pendingBank_;
        updateActiveWithPendingPreset();
