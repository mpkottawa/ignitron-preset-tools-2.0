...
// 2.42" OLED 128x64 displays commonly use an SSD1309 driver.
// 1.3" OLED 128x64 displays commonly use an SH1106 driver.
// Choose driver below, only one can be defined!

// #define OLED_DRIVER_SSD1306
// #define OLED_DRIVER_SSD1309
...
// Boot mode shortcuts
// When enabled, an external SPST switch can force AMP mode at boot.
// The switch is only sampled during startup; changing it later requires rebooting.
// Wire the GPIO through an external 10k pull-up to 3.3V, with the switch to GND.
#define ENABLE_AMP_MODE_ROCKER_SWITCH
const int AMP_MODE_SWITCH_PIN = 35;
